...
const ImageAnalyzer = () => {
    ...
    const { analyzeImage, loading, result, error, clearResult } = useImageAnalysis()
        ...
        <div className="image-analyzer">
            ...
            {result && result.analysis && (
                <div className="analysis-results">
                    ...
                    {safeGet(result, 'analysis.image_analysis.predictions') && (
                        <div className="image-predictions">
                            ...
                            {result.analysis.image_analysis.predictions.map((pred, index) => (
                                <div key={index} className="prediction-item">
                                    <span className="label">{pred.label || 'Unknown'}</span>
                                    <span className="confidence">
                                        {pred.score ? (pred.score * 100).toFixed(1) : '0.0'}%
                                    </span>
                                    <div className="confidence-bar">
                                        <div 
                                            className="confidence-fill"
                                            style={{ width: `${(pred.score || 0) * 100}%` }}
                                        />
                                    </div>
                                </div>
                            ))}
                    ...
                    {safeGet(result, 'analysis.text_analysis') && (
                        <div className="text-analysis">
                            <h5>Context Analysis:</h5>
                            
                            {/* Sentiment Analysis */}
                            {safeGet(result, 'analysis.text_analysis.sentiment.0') && (
                                <div className="sentiment">
                                    <strong>Sentiment:</strong> {result.analysis.text_analysis.sentiment[0].label || 'Unknown'}
                                    ({(result.analysis.text_analysis.sentiment[0].score * 100).toFixed(1)}%)
                                </div>
                            )}
                            
                            {/* Categories */}
                            {safeGet(result, 'analysis.text_analysis.categories.labels') && (
                                <div className="categories">
                                    <strong>Categories:</strong>
                                    {result.analysis.text_analysis.categories.labels.map((label, index) => {
                                        const score = safeGet(result, `analysis.text_analysis.categories.scores.${index}`, 0);
                                        return (
                    ...
                    {safeGet(result, 'analysis.combined_description') && (
                    ...
                    {/* Processing Info */}
                    {safeGet(result, 'analysis.processing_time') && (
